...
B = c / (2*radar_resolution);
t_chirp = 5.5 * 2 * radar_range / c;
slope = B / t_chirp;
...
Mix_M = reshape(Mix, [Nr, Nd]);
% TODO:
% Run the FFT on the beat signal along the range bins dimension (Nr) and
% normalize.
Mix_fft = fft(Mix_M, [], 1) / Nr;
% TODO:
...
for row1 = Tr + Gr + 1 : (Nr/2) - (Tr + Gr)
  for col1 = Td + Gd + 1 : (Nd) - (Td + Gd)
    %Create a vector to store noise_level for each iteration on training cells
    noise_level = zeros(1, 1);

    for row2 = row1 - (Tr + Gr) : row1 + (Tr + Gr)
      for col2 = col1 - (Td + Gd) : col1 + (Td + Gd)
        if (abs(row1 - row2) > Gr || abs(col1 - col2) > Gd)
